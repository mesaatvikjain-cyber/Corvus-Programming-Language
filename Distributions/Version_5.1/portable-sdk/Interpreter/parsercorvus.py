...
class Parser:
    ...
    def parse_statement(self):
        ...
        if tok.type in ('KEYWORD', 'ID') and tok.value == 'const':
            self.advance()
            self.match('SEMI')
            name = self.expect('ID').value
            self.expect('ASSIGN')
        ...
        if tok.type == 'KEYWORD' and tok.value == 'givout':
            ...
            val = None
            nxt = self.peek()
            if nxt and nxt.type not in ('RBRACKET', 'RBRACE', 'NEWLINE'):
                val = self.parse_expression()
    ...
    def parse_match(self):
        ...
        while True:
            ...
            if tok and tok.type == 'KEYWORD' and tok.value == 'case':
                ...
                if self.peek() and self.peek().type in ('LBRACKET', 'LBRACE'):
                    body = self.parse_block()
                else:
    ...
    def parse_var_or_const_decl(self):
        ...
        if nxt and nxt.type == 'KEYWORD' and nxt.value == 'const':
            self.advance()
            self.match('SEMI')  # optional semicolon: set const; x = 1 OR set const x = 1
            name = self.expect('ID').value
            self.expect('ASSIGN')
            val = self.parse_expression()
            return ConstDeclNode(name=name, value=val)
        ...
        # 1. set name = expr (inferred type)
        # 2. set type name = expr (optional semicolon)
        # 3. set type; name = expr (traditional)
        tok = self.peek()
        # Look ahead 2 tokens:
        next_tok = self.tokens[self.pos + 1] if self.pos + 1 < len(self.tokens) else None

        if tok and tok.type == 'ID' and next_tok and next_tok.type == 'ASSIGN':
            # Inferred type: set name = expr
        ...
        if tok and tok.type in ('TYPE', 'ID'):
            ...
            raise CorvusError(
                error_type="Corvus SyntaxError",
                message=f"Expected type specification or identifier after 'set', but got '{tok.value if tok else 'EOF'}'",
                line=tok.line if tok else 1,
                col=tok.column if tok else 1,
                suggestion="Use syntax: set <name> = <value> or set <type>; <name> = <value>"
            )

        self.match('SEMI')  # optional semicolon
        name = self.expect('ID').value
        val = None
        if self.match('ASSIGN'):
            val = self.parse_expression()
    ...
    def parse_if(self) -> IfNode:
        ...
        if not has_paren:
            old_stop = self.stop_at_block
            self.stop_at_block = True
            cond = self.parse_expression()
            self.stop_at_block = old_stop
        else:
            cond = self.parse_expression()
            self.expect('RPAREN')
