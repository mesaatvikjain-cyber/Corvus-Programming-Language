...
class CorvusClassConstructor:
    def __init__(self, name: str, body):
        self.name = name
        self.body = body

    def __call__(self, *args):
        from evaluatorcorvus import CorvusClass, Evaluator, Environment
        cls_obj = CorvusClass(self.name, self.body, Environment())
        ev = Evaluator(Environment())
        return cls_obj.instantiate(ev, args)
...
class BytecodeCompiler:
    ...
    def visit_ClassDeclNode(self, node: ClassDeclNode):
        constructor = CorvusClassConstructor(node.name, node.body)
        const_idx = self.code.add_const(constructor)
        self.code.emit(OpCode.LOAD_CONST, const_idx)
        name_idx = self.code.add_name(node.name)
        self.code.emit(OpCode.STORE_VAR, name_idx)
