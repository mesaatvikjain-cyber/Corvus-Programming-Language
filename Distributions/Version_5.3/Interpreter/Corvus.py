    ...
    try:
        ...
        if ai_fix:
            try:
                ...
                bot = RavenAI()
                healed_ok, report, healed_code = bot.auto_heal(filepath, error_msg=str(e), apply_in_place=True)
                print(report)
                if healed_ok:
                    print("\n[AI Self-Healing]: Re-executing auto-healed program...\n")
                    tokens = tokenize(healed_code)
                    parser = Parser(tokens)
                    ast = parser.parse()
                    if optimize:
                        from ast_optimizer import AstOptimizer
...
def run_vm(filepath: str, enable_jit: bool = False):
    ...
    try:
        ...
        machine = CorvusVM(enable_jit=enable_jit)
...
def main():
    ...
    # Bytecode Compiler (corvus compile <file.crv> [-o output.crvc] [--target wasm])
    if arg1 == "compile":
        if len(sys.argv) < 3:
            print("Usage: corvus compile <file.crv> [-o output.crvc] [--target wasm]")
            sys.exit(1)
        src_file = sys.argv[2]
        out_file = None
        if "-o" in sys.argv:
        ...
        if "--target" in sys.argv:
            t_idx = sys.argv.index("--target")
            if t_idx + 1 < len(sys.argv) and sys.argv[t_idx + 1].lower() == "wasm":
                try:
                    from wasm_backend import compile_corvus_to_wasm
                except ImportError:
                    from Interpreter.wasm_backend import compile_corvus_to_wasm
                compile_corvus_to_wasm(src_file, out_file)
                return
    ...
    if arg1 in ("--vm", "-vm"):
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        run_vm(sys.argv[2], enable_jit=False)
        return

    # Tiered JIT Direct Run (corvus --jit <file.crv | file.crvc>)
    if arg1 in ("--jit", "-jit"):
        if len(sys.argv) < 3:
            print("Usage: corvus --jit <file.crv | file.crvc>")
            sys.exit(1)
        run_vm(sys.argv[2], enable_jit=True)
        return

    # Corvus AI - RavenAI Assistant (corvus ai [ask | explain | fix | translate | chat])
    if arg1 == "ai":
        try:
            from ai_engine import RavenAI
        except ImportError:
        ...
        if subcmd in ("chat", "-c"):
            bot.chat()
        elif subcmd in ("ask", "-a"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai ask \"<question>\"")
            else:
                query = " ".join(sys.argv[3:])
                print(bot.ask(query))
        elif subcmd in ("explain", "-e", "summary"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai explain <file.crv | code>")
            else:
                target = sys.argv[3]
                print(bot.explain(target))
        elif subcmd in ("fix", "-f"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai fix <file.crv | code> [--apply]")
            else:
                target = sys.argv[3]
                apply_flag = "--apply" in sys.argv
                print(bot.fix(target, apply_in_place=apply_flag))
        elif subcmd in ("testgen", "-tg"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai testgen <file.crv> [-o output_test.crv]")
            else:
                target = sys.argv[3]
                out_f = None
                if "-o" in sys.argv:
                    o_idx = sys.argv.index("-o")
                    if o_idx + 1 < len(sys.argv):
                        out_f = sys.argv[o_idx + 1]
                print(bot.testgen(target, out_f))
        elif subcmd in ("doc", "-d"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai doc <file.crv> [-o output_docs.md]")
            else:
                target = sys.argv[3]
                out_f = None
                if "-o" in sys.argv:
                    ...
                    if o_idx + 1 < len(sys.argv):
                        out_f = sys.argv[o_idx + 1]
                print(bot.doc(target, out_f))
        elif subcmd in ("review", "-rv"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai review <file.crv | code>")
            else:
                target = sys.argv[3]
                print(bot.review(target))
        elif subcmd in ("export", "-exp"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai export <file.crv> [--target py|c]")
            else:
                target = sys.argv[3]
                target_lang = "py"
                if "--target" in sys.argv:
                    ...
                    if t_idx + 1 < len(sys.argv):
                        target_lang = sys.argv[t_idx + 1]
                print(bot.export_code(target, target_lang))
        elif subcmd in ("gen", "-g"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai gen \"<natural language prompt>\"")
            else:
                prompt = " ".join(sys.argv[3:])
                print(bot.gen(prompt))
        elif subcmd in ("refactor", "-r"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai refactor <file.crv> [-o output.crv]")
            else:
                target = sys.argv[3]
                out_f = None
                if "-o" in sys.argv:
                    ...
                    if o_idx + 1 < len(sys.argv):
                        out_f = sys.argv[o_idx + 1]
                print(bot.refactor(target, out_f))
        elif subcmd in ("model", "-m", "info"):
            print(bot.model_info())
        elif subcmd in ("complete", "-cp"):
            if len(sys.argv) < 4:
                print("Usage: corvus ai complete \"<code prefix>\"")
            else:
                prefix = " ".join(sys.argv[3:])
                print(bot.complete(prefix))
        elif subcmd in ("train", "-tr"):
            try:
                from train_raven_rlcf import train_rlcf
            except ImportError:
                from Interpreter.train_raven_rlcf import train_rlcf
            epochs = 15
            if len(sys.argv) > 3 and sys.argv[3].isdigit():
                epochs = int(sys.argv[3])
            train_rlcf(epochs=epochs)
        else:
            print(f"Unknown AI subcommand: {subcmd}")
            print("Available: corvus ai [chat | ask | explain | fix | translate | testgen | doc | review | export | gen | refactor | model | complete | train]")
        return

    # Corvus Package Manager (corvus pkg [init | add | install | list])
    if arg1 == "pkg":
        try:
            from package_manager import handle_pkg_cli
        except ImportError:
            from Interpreter.package_manager import handle_pkg_cli
        handle_pkg_cli(sys.argv[2:])
        return

    # Interactive Guided Tour (corvus tour)
    if arg1 == "tour":
        try:
            from tour import start_tour
        except ImportError:
            from Interpreter.tour import start_tour
        start_tour()
        return

    # Language Server Protocol Daemon (corvus lsp)
    if arg1 == "lsp":
        try:
            from lsp_server import start_lsp_server
        except ImportError:
            from Interpreter.lsp_server import start_lsp_server
        start_lsp_server()
        return
    ...
    if arg1 in ("--repl", "-r"):
        ...
        print("Corvus Language Launcher & Toolchain v5.3.0 (Cognitive Intelligence & Neural Self-Healing Ecosystem)")
        print("Usage:")
        ...
        print("  corvus --jit <file.crv>                  Execute with Tiered JIT bytecode compiler")
        print("  corvus tour                              Interactive in-terminal guided language tour")
        print("  corvus lsp                               Launch Language Server Protocol (LSP) daemon")
        print("  corvus pkg init [name]                   Initialize a new Corvus project manifest")
        print("  corvus pkg add <url|pkg>                 Add & download package from Git/ZIP to crv_modules/")
        print("  corvus pkg install                       Install all dependencies in corvus.json")
        print("  corvus pkg list                          List installed CPM packages")
        print("  corvus ai [chat]                         Launch interactive RavenAI Assistant")
        print("  corvus ai model                          Display RavenLM neural transformer specs & metrics")
        print("  corvus ai complete \"<prefix>\"            Autoregressive code completion with neural transformer")
        print("  corvus ai train [--epochs N]             Train or fine-tune RavenLM neural model")
        print("  corvus ai ask \"<question>\"               Ask RavenAI about Corvus syntax or concepts")
        print("  corvus ai explain <file.crv>             Analyze and explain code structure")
        print("  corvus ai fix <file.crv>                 Diagnose bugs and suggest automatic patches")
        print("  corvus ai translate <file.py>            Transpile Python source code to Corvus")
        print("  corvus ai testgen <file.crv> [-o out]    Generate automated unit test suite")
        print("  corvus ai doc <file.crv> [-o out.md]     Generate Markdown documentation & API specs")
        print("  corvus ai review <file.crv>              Static code analysis, linting & performance review")
        print("  corvus ai export <file.crv> [--target]   Reverse transpile Corvus to Python or C99")
        print("  corvus ai gen \"<prompt>\"                 Synthesize Corvus code from natural language")
        print("  corvus ai refactor <file.crv>            Auto-refactor and modernize Corvus code")
        print("  corvus compile <file.crv> [-o out]       Compile Corvus source to binary bytecode")
        print("  corvus compile <file.crv> --target wasm  Compile Corvus source to WebAssembly (.wat)")
