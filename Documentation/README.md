...
* **🧠 Deep Learning Neural AI & Cognitive Ecosystem (v5.2)**: **RavenLM** in-tree causal neural transformer language model (166,464 weights), autoregressive code completion (`corvus ai complete`), model inspection (`corvus ai model`), fine-tuning pipeline (`corvus ai train`), and zero-dependency dual inference (PyTorch + pure NumPy fallback).
* **⚡ Omni-Platform & Real-Time Ecosystem (v5.1)**: High-performance **Tiered JIT Compiler** (`corvus --jit`), **Real-Time Async WebSockets** (`web.ws_server`) & **Typed Channels** (`channel.new()`), **In-Browser Wasm Playground & IDE** (zero install), **RavenAI 3.0** (`ai gen` natural language synthesis & `ai refactor` code modernizer), **Corvus GameKit & Retro Audio Synthesizer** (`gamekit`, `audio` with 3 complete games: Flappy Crow, Space Invaders, Asteroids), and zero-dependency **Active-Record SQLite ORM** (`orm`) & **Reactive DataFrames** (`dataframe`).
