...
class AsmGeneratorLinux:
    ...
    def generate_from_ir(self, ir_program):
        """Code-generate NASM ELF64 assembly directly from optimized Three-Address Code (TAC) IR"""
    ...
    def generate(self, node):
        ...
        if node_type == "ProgramNode":
            ...
            self.text_lines.append(f"    mov rax, [var_{node.name}]")
            ...
            if node.value is not None:
                ...
                self.text_lines.append(f"    mov [var_{node.name}], rax")
            ...
            if isinstance(node.target, IdentifierNode):
                self.text_lines.append(f"    mov [var_{node.target.name}], rax")
                ...
                self.text_lines.append(f"    mov [var_{target_name}], rax")
            ...
            for i, param in enumerate(node.params):
                if i < len(sysv_regs):
                    self.text_lines.append(f"    mov [{param}], {sysv_regs[i]}")
                else:
            ...
            if node.method_name in ("length", "len"):
                ...
                if node.method_name == "load" and node.args:
                    ...
                    self.text_lines.append("    call dlopen")
                    ...
                    self.text_lines.append("    call dlsym")
                ...
                if node.method_name == "fly" and node.args:
                    ...
                    self.text_lines.append("    call pthread_create")
                    self.text_lines.append("    pop rax")
            ...
            if node.prompt is not None:
                ...
                self.text_lines.append(f"    lea rdi, [rel {lbl}]")
                self.text_lines.append("    mov rax, 0")
                self.text_lines.append("    call printf")

            self.text_lines.append("    lea rdi, [rel fmt_scan_int]")
            self.text_lines.append("    lea rsi, [rel input_buffer]")
            self.text_lines.append("    mov rax, 0")
    ...
    def build_full_asm(self):
        asm = []
        asm.append("; ========================================")
        asm.append("; Corvus NASM 64-bit Assembly Output (Linux ELF64 System V ABI)")
        ...
        asm.append("extern dlopen")
        asm.append("extern dlsym")
        asm.append("extern pthread_create")
