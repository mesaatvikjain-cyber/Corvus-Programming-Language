...
class RavenEngine:
    ...
    def model_info(self):
        ...
        vocab_sz = len(self._ai.tokenizer.vocab) if (self._ai and self._ai.tokenizer) else 198
        return {
            "name": "RavenLM-5.0",
            "parameters": 178752,
            ...
            "grammar_constraint": "Active (CFG Zero-Hallucination)",
            "backend": backend_name,
            "status": "loaded" if self._ai else "ready"
        }

    def check_grammar(self, text: str):
        """Validates brackets and CFG grammar state."""
        if self._ai and hasattr(self._ai, "constraint") and self._ai.constraint:
            st = self._ai.constraint.analyze_syntax_state(str(text))
            return {
                "balanced": len(st["bracket_stack"]) == 0 and not st["in_string"],
                "bracket_stack": st["bracket_stack"],
                "in_string": st["in_string"],
                "expected_close": st["expected_close"]
            }
        return {"balanced": True, "bracket_stack": []}

    def heal(self, target: str):
        """Autonomous self-healing diagnostic on broken Corvus code."""
        if self._ai and hasattr(self._ai, "auto_heal"):
            success, report, healed = self._ai.auto_heal(str(target))
            return {
                "success": success,
                "report": report,
                "healed": healed
            }
        return {"success": False, "report": "RavenAI unavailable", "healed": str(target)}
