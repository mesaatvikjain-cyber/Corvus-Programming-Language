...
class Evaluator:
    ...
    def _setup_builtins(self):
        ...
        # Corvus Native Async Channels (v5.1)
        try:
            from std_channel import channel_new
        except ImportError:
            from Interpreter.std_channel import channel_new
        self.global_env.define("chan", ModuleNamespace("chan", {
            "new": lambda cap=0: channel_new(cap)
        }), "any")
        self.global_env.define("channel", ModuleNamespace("channel", {
            "new": lambda cap=0: channel_new(cap)
        }), "any")
        ...
        raven_dict = {
            ...
            "check_grammar": lambda *args: _global_raven_engine.check_grammar(*args),
            "heal": lambda *args: _global_raven_engine.heal(*args)
        }
        ...
        if op == '@':
            ...
            if isinstance(left, list) and isinstance(right, list):
                if left and isinstance(left[0], list) and right and isinstance(right[0], list):
                    ...
                    c_b = len(right[0])
                    for row in left:
                        if not isinstance(row, list) or len(row) != c_a:
                            raise CorvusError(
                                error_type="Corvus MathError",
                                error_code="E0302",
                                message="Matrix multiplication '@' requires uniform rectangular 2D matrices.",
                                suggestion="Ensure all rows in the left matrix have equal length."
                            )
                    for row in right:
                        if not isinstance(row, list) or len(row) != c_b:
                            raise CorvusError(
                                error_type="Corvus MathError",
                                error_code="E0302",
                                message="Matrix multiplication '@' requires uniform rectangular 2D matrices.",
                                suggestion="Ensure all rows in the right matrix have equal length."
                            )
                    if c_a != r_b:
                        raise CorvusError(
                            error_type="Corvus MathError",
    ...
    def visit_ForNode(self, node: ForNode):
        ...
        try:
            ...
            raise CorvusError(
                error_type="Corvus TypeError",
                error_code="E0101",
                message=f"'{type(iterable).__name__}' object is not iterable.",
                suggestion="For loops require an iterable collection (such as list, tuple, dict, or string)."
            )
        self.loop_depth += 1
        try:
    ...
    def visit_GetNode(self, node: GetNode):
        ...
        if mod_name == "math":
            ...
            def file_append(path, content):
                safe_p = _validate_path(path)
                with open(safe_p, "a", encoding="utf-8") as f:
                    f.write(str(content))
                return True
            ...
            def _validate_http_url(url):
                ...
                if parsed.scheme.lower() not in {"http", "https"}:
                    raise ValueError(f"Insecure or invalid URL scheme '{parsed.scheme}'. Only http and https are allowed.")
                return parsed.geturl()

            def http_get(url):
                import urllib.request
                safe_url = _validate_http_url(url)
                with urllib.request.urlopen(safe_url, timeout=15) as req:
                    return req.read(10 * 1024 * 1024).decode("utf-8", errors="replace")

            def http_post(url, data=""):
                import urllib.request
                safe_url = _validate_http_url(url)
                encoded = str(data).encode("utf-8")
                req = urllib.request.Request(safe_url, data=encoded, headers={"Content-Type": "application/json"})
                with urllib.request.urlopen(req, timeout=15) as res:
                    return res.read(10 * 1024 * 1024).decode("utf-8", errors="replace")

            def http_download(url, filename):
                import urllib.request
                safe_url = _validate_http_url(url)
                safe_filename = os.path.abspath(str(filename))
                req = urllib.request.Request(safe_url)
                with urllib.request.urlopen(req, timeout=30) as resp, open(safe_filename, "wb") as out_f:
                    while True:
            ...
            def process_run(cmd):
                ...
                try:
                    ...
                    res = subprocess.run(args, shell=False, capture_output=True, text=True, timeout=30)
                except Exception:
                    res = subprocess.run(str(cmd), shell=True, capture_output=True, text=True, timeout=30)
        ...
        elif mod_name in ("web", "corvus_web"):
            try:
                from std_web import create_server, create_ws_server
            except ImportError:
                from Interpreter.std_web import create_server, create_ws_server
            web_symbols = {
                "server": create_server,
                "create_server": create_server,
                "ws_server": create_ws_server,
                "create_ws_server": create_ws_server
            }
            mod_obj = ModuleNamespace(mod_name, web_symbols)
            self.env.define(mod_name, mod_obj, "module")

        elif mod_name == "gamekit":
            try:
                from std_gamekit import create_world, PhysicsWorld, PhysicsBody
            except ImportError:
                from Interpreter.std_gamekit import create_world, PhysicsWorld, PhysicsBody
            gk_symbols = {
                "create_world": create_world,
                "PhysicsWorld": PhysicsWorld,
                "PhysicsBody": PhysicsBody
            }
            mod_obj = ModuleNamespace("gamekit", gk_symbols)
            self.env.define("gamekit", mod_obj, "module")

        elif mod_name == "audio":
            try:
                from std_audio import beep, play_tune
            except ImportError:
                from Interpreter.std_audio import beep, play_tune
            aud_symbols = {
                "beep": beep,
                "play_tune": play_tune
            }
            mod_obj = ModuleNamespace("audio", aud_symbols)
            self.env.define("audio", mod_obj, "module")

        elif mod_name == "orm":
            try:
                from std_orm import Model, QueryBuilder, create_model_class, connect_db
            except ImportError:
                from Interpreter.std_orm import Model, QueryBuilder, create_model_class, connect_db
            orm_symbols = {
                "Model": Model,
            ...
            mod_obj = ModuleNamespace("orm", orm_symbols)
            self.env.define("orm", mod_obj, "module")

        elif mod_name in ("dataframe", "df"):
            try:
                from std_dataframe import DataFrame, read_csv, from_records
            except ImportError:
                from Interpreter.std_dataframe import DataFrame, read_csv, from_records
            df_symbols = {
                "DataFrame": DataFrame,
                "read_csv": read_csv,
                "from_records": from_records
            }
            mod_obj = ModuleNamespace(mod_name, df_symbols)
            self.env.define(mod_name, mod_obj, "module")

        else:
            # 1. Check if module is already registered in global environment (e.g. graphics, audio, torch, tensorflow, numpy)
            if mod_name in self.global_env.values:
                self.env.define(mod_name, self.global_env.values[mod_name], "module")
                return

            # 2. Attempt to load a Corvus package from StdLib, corvus_modules/ or global package store
