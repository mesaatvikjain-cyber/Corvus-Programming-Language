    ...
    def _write_const(val: Any):
        if val is None:
            ...
            if val < -9223372036854775808 or val > 9223372036854775807:
                raise ValueError(f"Integer constant {val} exceeds 64-bit integer range (-2^63 to 2^63 - 1).")
            buf.append(2)
            ...
            try:
                ...
                raise ValueError(f"Float constant exceeds IEEE 754 double range: {err}")
...
def deserialize(data: bytes) -> CodeObject:
    ...
    def _read_const() -> Any:
        ...
        if tag == 0:
            ...
            offset += 4
            _ensure_bytes(length)
            obj = pickle.loads(data[offset:offset + length])
            offset += length
            return obj
