...
if [ "$CHOICE" == "0" ]; then
    ...
    if [ ! -f "$STANDALONE_INSTALLER" ]; then
        STANDALONE_INSTALLER="$SCRIPT_DIR/Distributions/Version_4.7/install_standalone_macos.sh"
    fi
    if [ ! -f "$STANDALONE_INSTALLER" ]; then
        STANDALONE_INSTALLER="$SCRIPT_DIR/Distributions/Version_4.6/install_standalone_macos.sh"
    fi
    if [ -f "$STANDALONE_INSTALLER" ]; then
